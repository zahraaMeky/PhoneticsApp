...
// }
 ...
 // check if exist
 $sql = "SELECT * FROM `quiz3` WHERE `words_list`='$words'";
//  echo $sql . "<br>";
 $result = mysqli_query($db, $sql);
 if (mysqli_num_rows($result) > 0) {
    echo "word";
    ...
    $sql = "INSERT INTO `quiz3`( `words_list`, `phonetic_id`) VALUES ('$words',$phonetic_id)";
    $result = mysqli_query($db, $sql);
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($items!=null) {
            ...
            //    $voice_path = uploadFile($phonetic_id, "voice", $voice);
               ...
               $SQL = "INSERT INTO `quiz3_content`(`quiz_id`, `order_`, `image_url`, `correct_word`) VALUES ($quiz_id,$order,'$image_path','$word')";
