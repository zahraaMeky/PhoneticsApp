...
$sql = "SELECT voice_url, image_url,id FROM quiz1_content  WHERE correct_word='$word' and quiz_id = '$QuizID' ";
...
// check if the file does exist
    ...
    $sql = "DELETE FROM quiz1_content WHERE id='$word_id'";
...
$sql = "SELECT `words_list` FROM quiz1 WHERE id=$QuizID";
$result = mysqli_query($db, $sql);
if (mysqli_num_rows($result) > 0) {
...
if($words_list_new == ""){
  $sql="DELETE FROM quiz1  WHERE id='$QuizID'";

}else{
  $sql="UPDATE quiz1 SET words_list = '$words_list_new'  WHERE id='$QuizID'";
