...
$check_phQuiz = "SELECT * FROM `quiz1_content` WHERE `correct_word` = '$word'";
$phqz_result = mysqli_query($db,$check_phQuiz);
if (mysqli_num_rows($phqz_result) > 0) {
    echo -1;
}else{
    //fetch word list and store it 
    $Fetch_word_list = "SELECT `words_list` FROM quiz1 WHERE id=$quiz_id";
    ...
    $Update_quiz1="UPDATE `quiz1` SET `words_list` = '$words_list_new'  WHERE `id`='$quiz_id'";
    //echo $sql;
    if (mysqli_query($db, $Update_quiz1)) {
      //echo "updated";
    } else {
...
if (($_FILES['image']['name']!="")){
        ...
        $update_Quiz2 = "UPDATE `quiz1_content` SET `correct_word` = '$word',`image_url`='$database_dir' WHERE `id`='$word_id'";
