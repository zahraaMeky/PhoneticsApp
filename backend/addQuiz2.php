...
if (mysqli_num_rows($result) > 0) {
 ...
 // check if exist
     $sql = "SELECT * FROM `quiz2` WHERE `word`='$word' AND phonetic_id=$phonetic_id";
    $result = mysqli_query($db, $sql);
    if (mysqli_num_rows($result) > 0) {
        echo 'word';
        die();
    ...
    // Where the file is going to be stored
        ...
        move_uploaded_file($temp_name,$path_filename_ext);
              ...
              // query process
            $sql = "INSERT INTO `quiz2`(`word`, `image`, `type`, `phonetic_id`) VALUES ('$word','$database_dir','$word_type',$phonetic_id)";
