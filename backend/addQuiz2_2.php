...
if (mysqli_num_rows($result) > 0) {
 ...
 // check if exist
     $sql = "SELECT * FROM `quiz2` WHERE `word`='$word' AND phonetic_id=$phoneticsID";
    $result = mysqli_query($db, $sql);
    if (mysqli_num_rows($result) > 0) {
        echo 'word';
        die();
    }else{
        $target_dir = "/var/www/html/phonetics_app/assets/images/$phonetic_name";
    ...
    // Where the file is going to be stored
        ...
        $database_dir = "/assets/images/". $phonetic_name . "/" .$filename.".".$ext;
        ...
        move_uploaded_file($temp_name,$path_filename_ext);
              // query process
            $sql = "INSERT INTO `quiz2`(`word`, `image`, `type`, `phonetic_id`) VALUES ('$word','$database_dir','$type',$phoneticsID)";
